(function(global){
  ...
  function criarComprovante(venda,dinheiro){
    ...
    const cancelada=String(venda.status_registro||"").toUpperCase()==="CANCELADA";
    return `<div class="comprovante">
      ...
      <section class="comprovante-titulo"><h2>${cancelada?"COMPROVANTE DE COMPRA — CANCELADA":"COMPROVANTE DE COMPRA"}</h2>${cancelada?`<div class="comprovante-situacao"><strong>VENDA CANCELADA</strong><br>${escapar(venda.motivo_cancelamento||"")}</div>`:""}<div class="comprovante-identificacao"><strong>Venda nº ${escapar(venda.visita_id??venda.id)}</strong><span>Data: ${escapar(quando.data)}</span><span>Hora: ${escapar(quando.hora)}</span></div></section>
  ...
  function textoComprovante(venda,dinheiro){
    ...
    if(String(venda.status_registro||"").toUpperCase()==="CANCELADA")venda={...venda,observacoes:`VENDA CANCELADA${venda.motivo_cancelamento?` — ${venda.motivo_cancelamento}`:""}${venda.observacoes?` | ${venda.observacoes}`:""}`};
